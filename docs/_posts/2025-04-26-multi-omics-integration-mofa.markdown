...
date: 2025-04-26
tags: multi-omics MOFA bioinformatics
---

Multi-omics integration aims to combine multiple layers of molecular data, such
...
to uncover hidden structures or patterns shared across different data types.

<!--more-->

### So, What Is Matrix Factorization?

Matrix factorization is a technique used to decompose a large, complex matrix
...
patterns or relationships observed in the original matrix.

Imagine you have a large matrix where columns represent samples, such as tumor
...
interpret how different samples relate to each other based on the raw data.

Matrix factorization aims to reduce this high-dimensional data into a
...
thought of as a vector or axis in this reduced space.

The second matrix produced through matrix factorization is the **activity
matrix**, which represents how much each sample contains of each factor. In
other words, it reflects the activity of the factors.

When you multiply the latent factor matrix by the activity matrix, you get an
approximation of the original matrix but with far fewer parameters. This allows
you to uncover hidden patterns in the data.

![Matrix factorization diagram]({{ '/assets/img/MOFA_image.png' | relative_url }})

In the previous example, we have an expression matrix (**A**), which is
decomposed into:

- **Weight matrix (W):** latent factors representing the major axes of variation
  in the data.
- **Activity matrix (H):** the activity or strength of each factor in each
  sample.

For instance, tumor samples might have a high activity score for a latent factor
...
biological processes are less active in normal tissue.

For the **weight matrix (W)**, the higher the weight, the more important a gene
...
directions in the data where tumor and normal tissues vary.

In a more technical sense, these factors are linear combinations of the original
...
By projecting samples onto these latent factors, you can gain a clearer
